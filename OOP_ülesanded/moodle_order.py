...
class Container:
    ...
    def volume_left(self) -> int:
        ...
        used = sum(o.total_volume for o in self.orders)
        return self.volume - used
    ...
    def aggregate_order(self, customer: str, max_items_quantity: int, max_volume: int):
        ...
        for item in self.order_items:
            if item.customer != customer:
                ...
                continue
            if (used_qty + item.quantity <= max_items_quantity and used_volume + item.total_volume <= max_volume):
                items.append(item)
