...
class Statistics:
    ...
    def __init__(self, filename: str):
        ...
        self.__games : list[Games] = []
        self.__play_session : list[PlaySession] = []
    ...
    def get(self, path: str):
        ...
        "/total/{result_type}" - kus {result_type} on string võimalike väärtustega points, places
                                või winner, funktsioon peab tagastama, mitu seda tüüpi mängu on mängitud
...
"/game/{name}/record-holder" - tagastab mängija (string), kes on mängus nimega game_name saanud enim punkte (ühe mängu jooksul), viigi korral tagastada see, kes selle tulemuse esimesena saavutas (seda funktsiooni kutsutakse vaid points mängu korral)
        """
        if path == "/players":
            return list(map(labda p: p.get_name(), self.__players))
        elif path == "/games":
            return list(map(lambda p: p.get_name(), ))